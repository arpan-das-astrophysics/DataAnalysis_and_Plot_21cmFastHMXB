...
#Usage> python slicplot.py 'filename'
...
for i in range(1,len(sys.argv)):
	...
	if basename(filename)[0:3]=='del':
		...
		fig = plt.figure(dpi=72,figsize=(45.0, 38.0))
		...
		frame1.set_xlabel(r'${\rm\longleftarrow %s \longrightarrow}$'%(label), fontsize=120)
		c_dens = sub_fig.imshow(slice,cmap=cmap,norm=norm)
		c_dens.set_clim(vmin=-150,vmax=30)
		c_bar = fig.colorbar(c_dens, orientation='vertical')
		c_bar.set_label(r'${\rm \delta T_b [\mathrm{mK}]}$', fontsize=120, rotation=-90, labelpad=140)
		tick_array = np.linspace(-150,30,7)
		...
		plt.title(r'${\rm Fiducial\, SED(T_{vir}=5\times 10^4 K)}$',fontsize=120,y=1.02)
		...
		sub_fig = fig.add_subplot(111)
		#Change the color here if you want
		cmap = LinearSegmentedColormap.from_list('mycmap', ['darkblue','red'])
		norm = MidpointNormalize(midpoint=0)
		frame1 = plt.gca()
		frame1.axes.get_xaxis().set_ticks([])
		frame1.axes.get_yaxis().set_ticks([])
