...
with h5py.File('redshift8.h5','r') as hf8:
	for j in range(len(redshift8_index)):
		...
		opacity_redshift8.append(opacity8[20])
...
with h5py.File('redshift10.h5','r') as hf10:
	for k in range(len(redshift10_index)):
		...
		opacity_redshift10.append(opacity10[20])
...
with h5py.File('redshift12.h5','r') as hf12:
	for l in range(len(redshift12_index)):
		...
		opacity_redshift12.append(opacity12[20])
...
with h5py.File('redshift14.h5','r') as hf14:
	for m in range(len(redshift14_index)):
		...
		opacity_redshift14.append(opacity14[20])
...
print stellarmass_redshift8/20.e+6
...
plt.scatter(halomass_redshift8, NHI_redshift8, c=sfr_redshift8, s=3500, cmap=cmap, vmin=np.amin(sfr), vmax=np.amax(sfr),marker='o',label=r'${\rm z=7.99}$')
plt.scatter(halomass_redshift10, NHI_redshift10, c=sfr_redshift10, s=3500, vmin=np.amin(sfr), vmax=np.amax(sfr), cmap=cmap,marker='^',label=r'${\rm z=9.95}$')
plt.scatter(halomass_redshift12, NHI_redshift12, c=sfr_redshift12, s=3500, vmin=np.amin(sfr), vmax=np.amax(sfr), cmap=cmap,marker='*',label=r'${\rm z=12.12}$')
plt.scatter(halomass_redshift14, NHI_redshift14, c=sfr_redshift14, s=3500, vmin=np.amin(sfr), vmax=np.amax(sfr), cmap=cmap,marker='D',label=r'${\rm z=14.78}$')

c_bar=plt.colorbar()
c_bar.set_label(r'${\rm log[SFR(M_\odot/yr)]}$', fontsize=120, rotation=-90, labelpad=140)
...
ax1.xaxis.set_label_text(r'${\rm Halo\, Mass(M_\odot)}$', fontsize = 120, color='black')
ax1.yaxis.set_label_text(r'${\rm \langle N_{HI}\rangle}$', fontsize = 120, color='black')
...
leg=ax1.legend(fancybox=True, shadow=True, fontsize=65,loc='lower right',scatterpoints = 1)
for l in leg.legendHandles:
	l.set_color('dimgrey')
plt.tight_layout()
plt.savefig('column_den.pdf')
